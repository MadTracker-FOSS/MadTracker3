...
#	endif
    if ((conf = (MTConfigFile*) mtinterface->getconf("Global", false)))
    {
        if (conf->setsection("MTAudio"))
        {
            ...
            if (output.maxcpu > 0.9f)
            {
                output.maxcpu = 0.9f;
            }
            if (output.maxcpu < 0.1f)
            {
                output.maxcpu = 0.1f;
            }
            if (output.mincpu > output.maxcpu)
            {
                output.mincpu = output.maxcpu - 0.1f;
            }
            if (output.mincpu < 0.0f)
            {
                output.mincpu = 0.0f;
            }
...
#	endif
    LOGD("%s - [Audio] Starting audio thread..."
             NL);
    running = true;
...
#	ifdef _DEBUG
    ...
    si->arraydelete(_dp);
#	endif
    LOGD("%s - [Audio] Uninitializing..."
             NL);
    status &= (~MTX_INITIALIZED);
    LOGD("%s - [Audio] Stopping audio thread..."
             NL);
    running = false;
    if (thread)
    {
    ...
    LOGD("%s - [Audio] Freeing all devices..."
             NL);
    deactivatedevices();
#	if defined(_WIN32)
...
void MTAudioInterface::activatedevices()
{
    ...
    MTTRY
        ...
        if (y)
        {
            ...
            for(x = 0; x < y; x++)
            {
                output.device[x]->master = (Track*) oi->newobject(MTO_TRACK, 0, x);
            }
            for(x = 0; x < y; x++)
            {
...
void MTAudioInterface::deactivatedevices()
{
    ...
    MTTRY
        ...
        for(x = 0; x < y; x++)
        {
            oi->deleteobject(output.device[x]->master);
        }
        x = mtinterface->getnummodules();
        while(x > 0)
        {
