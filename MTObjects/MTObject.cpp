...
MTObject::MTObject(MTObject *parent, mt_uint32 type, mt_int32 i):
    modified(0), lockread(0), lockwrite(0), modifying(0), parent(parent), owner(0), lastowner(0), objecttype(type),
    id(i), flags(0), color(MTColor(160, 160, 160, 0))
{
    if (parent)
    {
...
bool MTObject::lock(int flags, bool lock, int timeout)
{
    ...
    if ((parent) && (parent->lockread) && (parent->lockwrite))
    { return false; }
    if (lock)
    {
        if ((flags & MTOL_READ) && (lockread == 0))
        { critical = true; }
        if ((flags & MTOL_WRITE) && (lockwrite == 0))
        { critical = true; }
    }
    else
    {
        if ((flags & MTOL_READ) && (lockread == 1))
        { critical = true; }
        if ((flags & MTOL_WRITE) && (lockwrite == 1))
        { critical = true; }
    };
    if ((critical) && (output->ndevices))
    {
        if (!output->lock->lock(timeout))
        { return false; }
    }
    else
    { critical = false; }
...
void MTObject::setmodified(int value, int flags)
{
    if (value)
    {
        ...
        modifying++;
        if (parent)
        { parent->setmodified(value, flags); }
        modifying--;
    }
    else
    {
...
bool MTObjectWrapper::lock(void *object, int flags, bool lock, int timeout)
{
    ...
    if ((o.parent) && (o.parent->lockread) && (o.parent->lockwrite))
    { return false; }
    if (lock)
    {
        if ((flags & MTOL_READ) && (o.lockread == 0))
        { critical = true; }
        if ((flags & MTOL_WRITE) && (o.lockwrite == 0))
        { critical = true; }
    }
    else
    {
        if ((flags & MTOL_READ) && (o.lockread == 1))
        { critical = true; }
        if ((flags & MTOL_WRITE) && (o.lockwrite == 1))
        { critical = true; }
    };
    if ((critical) && (output->ndevices))
    {
...
void MTObjectWrapper::setmodified(void *object, int value, int flags)
{
    ...
    if (value)
    {
        ...
        o.modifying++;
        if (o.parent)
        { o.parent->setmodified(value, flags); }
        o.modifying--;
    }
    else
    {
