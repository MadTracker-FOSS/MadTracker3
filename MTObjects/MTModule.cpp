...
bool MTModule::addchannel(InstrumentInstance* c)
{
    ...
    {
        LOGD("%s - [Objects] WARNING: Cannnot add channel!"
                 NL);
        return false;
    };
...
void MTModule::delchannel(InstrumentInstance* c)
{
    ...
    {
        LOGD("%s - [Objects] ERROR: Invalid channel!"
                 NL);
        return;
    };
    if (c->caller)
    ...
    while((i = (RI*) ris->next()))
    {
        ...
        switch (i->i)
        {
            case RI_NOOP:
                ...
                break;
            case RI_CLEARBUFFER:
                FLOG3("%5d %3d CLEAR       %.8X"
                          NL, i->level, i->cpu, i->buffer);
                break;
            case RI_PREPROCESS:
                FLOG3("%5d %3d PREPROCESS  %s"
                          NL, i->level, i->cpu, i->track->name);
                break;
            case RI_PROCESS:
                FLOG3("%5d %3d PROCESS     %s"
                          NL, i->level, i->cpu, i->track->name);
                break;
            case RI_POSTPROCESS:
                FLOG3("%5d %3d POSTPROCESS %s"
                          NL, i->level, i->cpu, i->track->name);
                break;
            case RI_PROCESSNODE:
                FLOG3("%5d %3d PROCESSNODE %s"
                          NL, i->level, i->cpu, i->node->name);
                break;
            case RI_AMPLIFYBUFFER:
                FLOG4("%5d %3d AMPLIFY     %.8X %f"
                          NL, i->level, i->cpu, i->buffer, i->factor);
                break;
            case RI_MIXBUFFER:
                FLOG4("%5d %3d MIX         %.8X %.8X"
                          NL, i->level, i->cpu, i->buffer, i->dest);
                break;
            default:
                break;
        };
...
#					ifdef _DEBUG
                    if (log)
                        FLOG3("%d  %.8X  %.8X"
                                  NL, y, A(tmpbuf, TmpBuf)[e.id]->buffer[y], outputs[y]);
#					endif
                };
#				ifdef _DEBUG
                if (log)
