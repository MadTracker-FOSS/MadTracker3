...
inline bool cliprect(MTRect& sel, MTRect& clip)
{
    ...
    if (sel.right > clip.right)
    {
        sel.right = clip.right;
    }
    if (sel.right <= sel.left)
    {
        return false;
    }
    if (sel.bottom > clip.bottom)
    {
        sel.bottom = clip.bottom;
    }
...
{
    if ((sel.right < clip.left) || (sel.bottom < clip.top) || (sel.left > clip.right) || (sel.top > clip.bottom))
    {
        return false;
    }
    return true;
...
inline int calcpitch(int width, int bitcount, int granbits)
{
    return (width * ((bitcount + 7) >> 3) >> granbits) << granbits;
};
