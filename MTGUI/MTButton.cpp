...
MTButton::MTButton(int tag, MTWinControl *p, int l, int t, int w, int h):
    MTControl(MTC_BUTTON, tag, p, l, t, w, h), imageindex(-1), modalresult(MTDR_NULL), shortcut(0), hotkey(0),
    autosize(false), downtime(0), hotkeyoffset(-1)
{
...
int MTButton::getautowidth()
{
    ...
    if (caption[0])
    {
        if (!skin->gettextsize(this, caption, -1, &bs))
        { return width; }
        if (imageindex >= 0)
        { bs.x += sysimages.iw + 2; }
    }
