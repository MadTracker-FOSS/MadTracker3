...
/**
 * Notice by irrenhaus3: This file is scary, and it's over 2700 lines long.
 * Be VERY afraid.
 */
        ...
        case MTCM_MOUSEDOWN:
                    ...
                    if (cctrl.guiid != MTC_MENUITEM)
                    {
                        ...
                        if (design)
                        {
                            if (msg.button == DB_LEFT)
                            {
                                ...
                                {
                                    cctrl.switchflags(MTCF_SELECTED, (cctrl.flags & MTCF_SELECTED) ? false : true);
                                }
                                else
                                {
            ...
            if (design)
            {
                if (msg.button == DB_LEFT)
                {
                    if (msg.buttons & DB_SHIFT)
                    {
                        switchflags(MTCF_SELECTED, (flags & MTCF_SELECTED) ? false : true);
                    }
...
bool MTWinControl::fillcolor(int x, int y, int w, int h, int color, int opacity)
{
    if (mb)
    {
        return mb->fill(box + x, boy + y, w, h, color, opacity);
    }
    else
    {
        screen->setwindow(this);
