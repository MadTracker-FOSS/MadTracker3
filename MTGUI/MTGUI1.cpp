...
MTControl* MTGUIInterface::newcontrol(int type, int tag, MTWinControl* parent, int l, int t, int w, int h, void* param)
{
    ...
    switch (type)
    {
        ...
        case MTC_OSWINDOW:
            ...
            break;
        default:
            FLOGD1("%s - [GUI] ERROR: Unknown object type %08X!"
                       NL, type);
            break;
    };
    if (res)
...
void MTGUIInterface::deltimer(MTControl* ctrl, int timerid)
{
    ...
    if (t->ctrl != ctrl)
    {
        LOGD("%s - [GUI] ERROR: Wrong timer/control association!"
                 NL);
        LEAVE();
        return;
    };
...
int MTGUIInterface::synchronize(MTSync* param)
{
    ...
    else
    {
        ...
        {
            nsync--;
            FLOGD1("%s - [GUI] ERROR: MTGUIInterface::synchronize(MTSync*) error %d"
                       NL, GetLastError());
            return -1;
        };
    };
...
MTGUIInterface::WindowProc(HWND
...
if (msg==WM_CREATE){
SendMessage(wnd, WM_SETICON, ICON_BIG, (LPARAM)

LoadImage(instance, MAKEINTRESOURCE(1), IMAGE_ICON,

0,0,LR_DEFAULTSIZE));

SendMessage(wnd, WM_SETICON, ICON_SMALL, (LPARAM)
...
FLOGD1("%s - [GUI] %s focus"
           NL, (gi->active) ? "Gaining" : "Loosing");
di->
...
FLOGD2("%s - [GUI] Resizing desktop to %d x %d"
           NL, r.right, r.bottom);
...
if ((newclick-lastclick<=doubleclick) && (
...
buttons = WinButtons(wparam & 0xFFFF);
