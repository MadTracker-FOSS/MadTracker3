...
proc = (WNDPROC) GetWindowLong((HWND)
...
additem(lparam, osproc

);

SetWindowLong((HWND)

lparam,GWL_WNDPROC,(LONG)CustomProc);
...
SendMessage(fwd, msg, wparam, lparam
...
right = osproc->oswnd->width;

r.

bottom = osproc->oswnd->height;
...
MTOSWindow::mGetUpdateRect(HWND
...
0,0,osproc->oswnd->width,osproc->oswnd->height);
return
...
lastproc->oswnd->parent->

message(msg);

lastproc->oswnd->parent->dsk->
...
right = nXDest + nWidthDest;

msg.dr.

bottom = nYDest + nHeightDest;
